...
export default function Payouts() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const p = profiles[0];
      if (!p) { const byEmail = await base44.entities.ReferralProfile.filter({ email: user.email }); setProfile(byEmail[0]); }
      else setProfile(p);

      if (p || profiles[0]) {
        const pp = await base44.entities.PaymentProfile.filter({ user_id: (p || profiles[0]).id });
        if (pp[0]) { setPaymentProfile(pp[0]); setForm({ recipient_name: pp[0].recipient_name || "", bank_name: pp[0].bank_name || "", bik: pp[0].bik || "", account_number: pp[0].account_number || "", card_number: pp[0].card_number || "" }); }
      }
      setLoading(false);
    };
    load();
  ...
  const handleSave = async () => {
    ...
    try {
      if (paymentProfile) {
        ...
        await base44.entities.PaymentProfile.create({ user_id: profile.id, ...form, verification_status: "pending_review", payment_method: "bank_transfer" });
      }
